...
(function($) {
  ...
  Flatdoc.github = function(repo, filepath, ref) {
    var url;
    if (filepath) {
      url = 'https://util.github.com/repos/'+repo+'/contents/'+filepath;
    } else {
      url = 'https://util.github.com/repos/'+repo+'/readme';
    }
    if (ref) {
      url += '?ref='+ref;
    }
  ...
  Flatdoc.bitbucket = function(repo, filepath, branch) {
    ...
    var url = 'https://bitbucket.org/util/1.0/repositories/'+repo+'/src/'+branch+'/'+filepath;
